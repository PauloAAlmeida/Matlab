...
plot(z,NUT(length(t)-pontos:length(t)),'.','Color',[0.54, 0.2 ,0.143],'Markersize',point_size)
...
plot(z,PICO(length(t)),'.','Color',[0.776, 0.424 ,0.11],'Markersize',point_size)
hold on
grid on
ylabel('\bf{PICO}')
...
plot(z,NANO(length(t)-pontos:length(t)),'.','Color',[0, 0.502 ,0],'Markersize',point_size)
 hold on
 grid on
...
plot(z,NANOZOO(length(t)-pontos:length(t)),'b.','Markersize',point_size)
hold on
grid on
 ylabel('\bf{NANOZ}')
...
plot(z,BACT(length(t)-pontos:length(t)),'r.','Markersize',point_size)
hold on
grid on
 ylabel('\bf{BACT }')
...
plot(z,DET(length(t)-pontos:length(t)),'k.','Markersize',point_size)
hold on
grid on
ylabel('\bf{DET}')
