...
y=[ci zeros(1,length(t)-1); ci zeros(1,length(t)-1); ...
    ...
    ci zeros(1,length(t)-1);ci zeros(1,length(t)-1);...
    0 zeros(1,length(t)-1); 0 zeros(1,length(t)-1)]; % cria a matriz com os que vão receber os valores de y para cada tempo (cada linha dessa matriz é uma dimensao do problema edo)
...
plot(NUT_TOTAL,PICO(length(t)),'.','Color',[0.776, 0.424 ,0.11],'Markersize',point_size)
hold on
grid on
ylabel('\bf{PICO}')
...
plot(NUT_TOTAL,NANO(length(t)-pontos:length(t)),'.','Color',[0, 0.502 ,0],'Markersize',point_size)
 hold on
 grid on
...
plot(NUT_TOTAL,NANOZOO(length(t)-pontos:length(t)),'b.','Markersize',point_size)
hold on
grid on
 ylabel('\bf{NANOZ}')
...
plot(NUT_TOTAL,BACT(length(t)-pontos:length(t)),'r.','Markersize',point_size)
hold on
grid on
 ylabel('\bf{BACT }')
