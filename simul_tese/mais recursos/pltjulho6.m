...
k=3;%tamanho do passo predador 1&2&3
...
%Predador 2
p2x=-100+(100--100)*rand;
p2y=-100+(100--100)*rand;
%Predador 3
p3x=-100+(100--100)*rand;
p3y=-100+(100--100)*rand;
...
for i=2:n+1
   ...
   phi=180; % variação do angulo de mudança de direção
   phi1=180;
   ...
   pr1yi(i:i,1)=pr1yi(i-1,1)+(k*sin((thp1i(i-1,1))*pi/180));
       %predador 2 
   pr2xi(i:i,1)=pr2xi(i-1,1)+(k*cos((thp2i(i-1,1))*pi/180));
   pr2yi(i:i,1)=pr2yi(i-1,1)+(k*sin((thp2i(i-1,1))*pi/180));
   %predador 3
   pr3xi(i:i,1)=pr3xi(i-1,1)+(k*cos((thp3i(i-1,1))*pi/180));
   pr3yi(i:i,1)=pr3yi(i-1,1)+(k*sin((thp3i(i-1,1))*pi/180));
...
%montagem das matrizes para regras de detecção
   ...
   Rper1= 35; %"raio perceptual" presa 1
   Rper2= 35; %"raio perceptual" presa 2
   Rper3= 35; %"raio perceptual" presa 3
