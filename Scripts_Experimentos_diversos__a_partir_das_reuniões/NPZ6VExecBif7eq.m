...
plot(NUT_TOTAL,PICO(length(t)-pontos:length(t)),'.','Color',[0.776, 0.424 ,0.11],'Markersize',point_size)
hold on
grid on
ylabel('\bf{PICO}')
...
plot(NUT_TOTAL,MICROZ(length(t)-pontos:length(t)),'k.','Markersize',point_size)
hold on
grid on
ylabel('\bf{MICRO}')
