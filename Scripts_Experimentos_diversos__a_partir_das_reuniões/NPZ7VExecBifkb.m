...
plot(k_B,NUT(length(t)-pontos:length(t)),'.','Color',[0.54, 0.2 ,0.143],'Markersize',point_size)
...
plot(k_B,PICO(length(t)),'.','Color',[0.776, 0.424 ,0.11],'Markersize',point_size)
hold on
grid on
ylabel('\bf{PICO}')
...
plot(k_B,NANO(length(t)-pontos:length(t)),'.','Color',[0, 0.502 ,0],'Markersize',point_size)
 hold on
 grid on
...
plot(k_B,NANOZOO(length(t)-pontos:length(t)),'b.','Markersize',point_size)
hold on
grid on
 ylabel('\bf{NANOZ}')
...
plot(k_B,BACT(length(t)-pontos:length(t)),'r.','Markersize',point_size)
hold on
grid on
ylabel('\bf{BACT }')
...
plot(k_B,DET(length(t)-pontos:length(t)),'k.','Markersize',point_size)
hold on
grid on
ylabel('\bf{DET}')

subplot(NL,NC,7)  % DET X TIME
plot(k_B,MICROZ(length(t)-pontos:length(t)),'k.','Markersize',point_size)
hold on
grid on
ylabel('\bf{MICROZ}')
