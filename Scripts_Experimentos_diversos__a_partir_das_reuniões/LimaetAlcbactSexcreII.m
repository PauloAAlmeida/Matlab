...
function dydt = LimaetAlcbactSexcreII(t,y)
...
lambda = 0.035;        % coef. de atenuação da luz - em m-1 (*)
z =25;                  % Profundidade (**)
...
% Segundo Lima et al. Ipar = Izero . teta . exp(-lambda . z )
 %% teta=fração da irradiação que esta disponivel para o plancton
 %% lambda = coeficiente de atenuação da luz

% Segundo Sarmiento  I(z) = Izero . exp (- K . z) 
 % Izero = fpar . f (CL) . f(tau) . In ----> In=irradiação superfície meio dia // f(tau) = entre 0 e 1 conforme a hora
...// f(CL)=impacto das nuvens na irradiação // fpar = fração da irradiação que esta no comprimento de onda que pode ser capturada pela clorofila no plancton.
    
 % K = kw + kp . P + kx ----> kw= coef. de atenuação da agua (clear
 % water=0.4) // kp = constante do efeito da densidade do plancton na
 % penetração da luz (atenuação da luz pela biomassa) // consante de atenuação da luz devido a outras
 % particulas
 
 % ---> Então : teta = fpar // lambda = kw
...
%Mesoz
G_3p1 = (phi3_p1*NANOF^2)/(K_z3*(phi3_p1*NANOF+phi3_z2*MICROZ+phi3_d*DET)+(phi3_p1*NANOF^2+phi3_z2*MICROZ^2+phi3_d*DET^2));
G_3z2 = (phi3_z2*NANOF^2)/(K_z3*(phi3_p1*NANOF+phi3_z2*MICROZ+phi3_d*DET)+(phi3_p1*NANOF^2+phi3_z2*MICROZ^2+phi3_d*DET^2));
G_3d = (phi3_d*NANOF^2)/(K_z3*(phi3_p1*NANOF+phi3_z2*MICROZ+phi3_d*DET)+(phi3_p1*NANOF^2+phi3_z2*MICROZ^2+phi3_d*DET^2));
...
%Mesoz
G_3 = (phi3_p1*NANOF^2)/(K_z3*(phi3_p1*NANOF+phi3_z2*MICROZ+phi3_d*DET)+(phi3_p1*NANOF^2+phi3_z2*MICROZ^2+phi3_d*DET^2));
