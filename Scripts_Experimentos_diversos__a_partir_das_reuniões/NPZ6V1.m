...
g0 = 0.01;           % NANOZ->PICO (* tabela 1, valor de Phip - No artigo, 'g' é uma função do tipo M.Menten)
g1 = 1.6;             % NANOZ->BACT (**)
g2 = 3;                % DET->BACT (**)
...
% SISTEMA 'NPZD 6V'
   ...
   dydt(3,1)  = U_1 * NANOF - dp1 * NANOF ; %NANOF
   
   dydt(4,1) =  c0 * g0 * PICO * NANOZOO + c1 * g1 * BACT * NANOZOO - dz1 * NANOZOO ; %NANOZ

   dydt(5,1) = c2 * g2 * DET * BACT - g1 * BACT * NANOZOO  - db * BACT ; %BACT
   
   dydt(6,1) =  ( 1 - c0 ) * g0 * PICO * NANOZOO + ( 1 - c2 )   * g2   * DET  * BACT +( 1 - c1 ) *  g1 * BACT * NANOZOO   + dp0 * PICO + dz1 * NANOZOO + dp1 * NANOF - g2 * DET * BACT; %DET                                                                            ;
    %DET
