...
g0 = 0.7*1;           % NANOZ->PICO (* tabela 2, valor de Phip - No artigo, 'g' é uma função do tipo M.Menten)
g1 = 1*1;             % NANOZ->BACT (**)
%g2 = 8;              % DET->BACT (**)
g3 = 0.35*1;               % NANOZ->NANOF (* tabela 2)
g4 = 0.45*1  ;            % MICROZ-NANOZ (*tabela 2)
...
% SISTEMA 'NPZD 7V'
   ...
   dydt(6,1) =  ( 1 - c0 ) * g0 * PICO * NANOZOO + ( 1 - c2 )   * g2   * DET  * BACT +( 1 - c1 ) *  g1 * BACT * NANOZOO +( 1 - c3 ) *  g3 * NANOF * NANOZOO   + ( 1 - c4 ) *  g4 * NANOZOO * MICROZ + dp0 * PICO + dz1 * NANOZOO +  dz2 * MICROZ + dp1 * NANOF + dd * BACT - g2 * DET * BACT; %DET                                                                            ;
   
   dydt(7,1) = c4 * g4 * NANOZOO * MICROZ - dz2 * MICROZ ; %MICROZ
